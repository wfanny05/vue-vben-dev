# 需求：基于 Vben Admin (apps/web-ele) 开发菜单管理页面

## 核心背景

在 Vben Admin 项目的 `apps/web-ele` 应用下开发菜单管理页面，严格遵循 Vben Admin 目录规范，同时指定组件库使用规则，按功能拆分独立组件，后端通过 Nitro 模拟接口。
...
src/views/system/menu/ ├── index.vue # 页面入口（整合所有子组件）├── components/│ ├── MenuTable.vue # 菜单表格组件（基于 vex-table）│ ├── MenuEditor.vue # 菜单新增 / 编辑弹窗（基于 el-form + IconPicker）│ └── SearchForm.vue # 搜索表单组件（基于 el-form）
